...
export default function StyleQuizPage() {
  ...
  const filteredResults = useMemo(() => {
    const byName = quiz.results.filter((item) =>
      item.costumeName.toLowerCase().includes(nameFilter.trim().toLowerCase())
    )

    if (sortBy === "price_asc") {
      return [...byName].sort((a, b) => Number(a.price) - Number(b.price))
    }

    if (sortBy === "price_desc") {
      return [...byName].sort((a, b) => Number(b.price) - Number(a.price))
    }
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-4 py-4">
      <Modal
        ...
        title="Tiếp tục bài quiz đang làm dở?"
        okText="Tiếp tục"
        cancelText="Bắt đầu mới"
        onOk={quiz.restoreDraft}
        onCancel={quiz.discardDraftAndStartNew}
        closable={false}
        maskClosable={false}
      >
        ...
        <div className="space-y-5 rounded-3xl border border-pink-200 bg-white p-6 shadow-sm md:p-8">
          ...
          <div className="flex justify-center">
            <button
              ...
              className="group relative h-11 rounded-2xl border border-pink-200 bg-gradient-to-br from-white via-pink-50 to-rose-100 px-6 text-sm font-normal text-slate-800 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-pink-300 hover:from-pink-100 hover:to-purple-100 hover:shadow-[0_12px_24px_rgba(236,72,153,0.14)] disabled:cursor-not-allowed disabled:opacity-50"
            >
              <span
                aria-hidden="true"
      ...
      {!quiz.surveyLoading && quiz.screen === "checkpoint" && (
        <div className="space-y-6 rounded-3xl border border-pink-200 bg-gradient-to-r from-pink-100 via-rose-50 to-pink-100 p-6 shadow-sm md:p-8">
          ...
          <h2 className="text-2xl font-semibold leading-snug text-slate-700 md:text-3xl">
            Hệ thống đã quét được <span className="font-extrabold text-pink-600">70%</span> bản ngã của bạn và xếp bạn vào nhóm{" "}
            <span className="font-extrabold text-pink-600">{quiz.archetypeProfile.name}</span>. Bạn muốn xem kết quả ngay hay test thêm 7 câu để phân tích chi tiết{" "}
            <span className="font-extrabold text-pink-600">100%</span>?
          </h2>

          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
            <button
              type="button"
              onClick={quiz.viewResultNow}
              className="h-11 rounded-2xl border border-pink-200 bg-white px-6 text-sm font-normal text-slate-600 shadow-sm transition-all duration-300 hover:border-pink-300 hover:bg-pink-50"
            >
              Xem kết quả ngay
            </button>
            <button
              type="button"
              onClick={quiz.continueDeepAnalysis}
              className="group relative h-11 rounded-2xl border border-pink-200 bg-gradient-to-br from-white via-pink-50 to-rose-100 px-6 text-sm font-normal text-slate-800 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-pink-300 hover:from-pink-100 hover:to-purple-100 hover:shadow-[0_12px_24px_rgba(236,72,153,0.14)]"
            >
              <span
                aria-hidden="true"
      ...
      {quiz.screen === "result" && quiz.results.length > 0 && (
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="h-full rounded-3xl border border-pink-200 bg-gradient-to-br from-pink-50 to-white p-6 shadow-sm">
            ...
            <div className="mt-5">
              <p className="text-sm font-semibold text-pink-600">Bảng màu phù hợp:</p>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                {quiz.archetypeProfile.color_palette.map((hex) => (
                  <span
                    key={hex}
                    title={hex}
                    className={[
                      "h-7 w-7 rounded-full border border-white shadow ring-1 ring-pink-100",
                      SWATCH_CLASS_BY_HEX[hex.toUpperCase()] ?? "bg-pink-200",
                    ].join(" ")}
                  />
                ))}
              </div>
            ...
            <div className="mt-5">
              <p className="text-sm font-semibold text-pink-600">Nhân vật nổi bật:</p>
              <div className="mt-2 flex flex-wrap gap-2">
                {quiz.archetypeProfile.famousCharacters.map((name) => {
                  const searchUrl = `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(`${name} anime character`)}`
                  return (
                    <Tooltip key={name} title="Mở Google Images để xem nhân vật">
                      <a
                        href={searchUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="rounded-full border border-pink-200 bg-white px-3 py-1 text-xs font-semibold text-pink-600 transition hover:border-pink-400 hover:bg-pink-50"
                      >
                        {name}
                      </a>
                    </Tooltip>
            ...
            <div className="mb-4 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <span className="text-xs text-slate-500">{filteredResults.length} gợi ý phù hợp</span>
                </div>
                <button
                  type="button"
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <Select
                  ...
                  options={[
                    { value: "similarity", label: "Khớp nhất (Similarity)" },
