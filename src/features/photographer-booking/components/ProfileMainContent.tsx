          ...
          {activeTab === 'Đánh giá' && (
            <div className="space-y-6">
              ...
              {!reviewsLoading && !reviewsError && providerReviews.length === 0 && (
                <div className="rounded-3xl border border-cosmate-lavender-border bg-white p-10 text-center text-sm text-cosmate-lavender-muted">
                      ...
                      <div className="flex justify-center mb-2">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`w-4 h-4 ${
                              s <= Math.round(reviewStats.avg) ? 'fill-cosmate-star text-cosmate-star' : 'text-cosmate-icon-dim'
                            }`}
                          />
                        ))}
                      </div>
                    ...
                    <div key={review.id} className="bg-white border border-cosmate-lavender-border rounded-3xl p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-4">
                          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-cosmate-lavender-surface text-sm font-bold text-cosmate-text-soft">
                            C
                          </div>
                          <div>
                            <h4 className="font-bold text-cosmate-ink">Cosplayer</h4>
                            <div className="flex gap-0.5">
                              {[1, 2, 3, 4, 5].map((s) => (
                                <Star
                                  key={s}
                                  className={`w-3 h-3 ${
                                    s <= review.rating ? 'fill-cosmate-star text-cosmate-star' : 'text-cosmate-icon-dim'
                                  }`}
                                />
                              ))}
                            </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {review.images.map((img) => (
                            <img
                              ...
                              alt=""
                              className="h-20 w-20 rounded-lg object-cover border border-cosmate-lavender-border"
                            />
                          ))}
                        </div>
                      )}
