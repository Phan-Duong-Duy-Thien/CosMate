...
const routeOptions = [
  {
    label: 'Khu vực Admin', options: [
      ...
      { label: '/admin/reviews (Duyệt đánh giá)', value: '/admin/reviews' },
...
const iconOptions = [
  ...
  { label: '⭐ Duyệt đánh giá (Star)', value: 'star' },
  ...
  { label: '⚙️ Cấu hình', value: 'settings' },
];
...
export default function AdminMenusPage() {
  ...
  const expandedRowRender = (menu: { id: string; menuItems?: unknown[] }) => {
    ...
    const sortedItems = menu.menuItems
      ? [...(menu.menuItems as any[])].sort((a, b) => (a.displayOrder ?? 0) - (b.displayOrder ?? 0))
      : [];
    return <Table columns={itemColumns} dataSource={sortedItems} pagination={false} rowKey="id" size="small" />;
  };
