...
import {
  getAdminAiTokenPurchaseById,
  getAdminAiTokenPurchases,
} from '../api/adminAiTokenPurchases.api';
import type { AiTokenPurchase } from '../types';
import { VI } from '@/shared/i18n/vi';

export function useAdminAiTokenPurchases() {
  ...
  const fetchPurchases = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const data = await getAdminAiTokenPurchases();
      setPurchases(Array.isArray(data) ? data : []);
    } catch {
      if (!silent) {
        message.error(VI.admin.aiTokenPurchases.loadError);
        setPurchases([]);
      }
    } finally {
  ...
  const fetchDetail = useCallback(
    async (id: number, fallback?: AiTokenPurchase) => {
      ...
      try {
        const data = await getAdminAiTokenPurchaseById(id);
        setDetail(data);
        return data;
      } catch {
        if (fallback) {
          setDetail(fallback);
          message.warning(VI.admin.aiTokenPurchases.detailFallback);
          return fallback;
        }
        message.error(VI.admin.aiTokenPurchases.detailError);
        setDetail(null);
        return null;
      } finally {
