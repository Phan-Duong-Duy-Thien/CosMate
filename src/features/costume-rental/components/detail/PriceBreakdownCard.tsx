  ...
  <div className="rounded-2xl border-[4px] border-indigo-950 bg-[#fffbeb] p-3 shadow-[8px_8px_0_0_rgba(30,27,75,0.5)]">
    ...
    <div className="mt-2 space-y-1.5 text-sm font-semibold text-indigo-900/85">
      ...
      {quote.accessoryTotal > 0 && (
        <div className="flex items-center justify-between">
          <span>Phụ kiện</span>
          <span className="font-bold text-indigo-950">{formatVnd(quote.accessoryTotal)}</span>
        </div>
      )}
      {quote.surchargesTotal > 0 && (
        <div className="flex items-center justify-between">
          <span>Phụ phí</span>
          <span className="font-bold text-indigo-950">{formatVnd(quote.surchargesTotal)}</span>
        </div>
      )}
      <div className="flex items-center justify-between">
        <span>Tiền cọc</span>
        <span className="font-bold text-indigo-950">{formatVnd(quote.deposit)}</span>
      </div>
    </div>
