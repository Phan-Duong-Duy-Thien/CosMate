...
import { AiInsufficientTokenModal } from '@/shared/components/AiInsufficientTokenModal'
import { useAiTokenGuard } from '@/shared/hooks/useAiTokenGuard'
...
export default function Phase1BasicInfoForm({ onSubmit, loading, error, disabled }: Props) {
  ...
  const { handleTokenError, goToTopUp } = useAiTokenGuard({ onInsufficientToken: () => setInsufficientTokenOpen(true) })
  ...
  const handleFinish = async (values: FormValues) => {
    ...
    const rangeValues = values as FormValues & { heightMin?: number; heightMax?: number; weightMin?: number; weightMax?: number }
    const hasFullRange = Number.isFinite(rangeValues.heightMin) && Number.isFinite(rangeValues.heightMax) && Number.isFinite(rangeValues.weightMin) && Number.isFinite(rangeValues.weightMax)
    const sizeString = hasFullRange ? `${values.size} (${rangeValues.heightMin}-${rangeValues.heightMax}cm, ${rangeValues.weightMin}-${rangeValues.weightMax}kg)` : values.size
  ...
  return (
    <>
      {moderationError && <AILoadingMascot type="moderation" onClose={() => setModerationError(null)} />}
      <AiInsufficientTokenModal open={insufficientTokenOpen} onCancel={() => setInsufficientTokenOpen(false)} onTopUp={goToTopUp} />

      <Form form={form} layout="vertical" initialValues={{ characterIds: [] }} onFinish={handleFinish} disabled={disabled || loading} style={{ maxWidth: 640, margin: '0 auto' }}>
        {error && <Form.Item><Alert type="error" message={error} showIcon /></Form.Item>}
        ...
        <Form.Item label="Nhân vật Anime / Game" name="characterIds">
          <Select mode="multiple" showSearch allowClear placeholder="Chọn nhân vật" loading={isCharactersLoading} optionFilterProp="title" filterOption={(input, option) => {
        ...
        <Card size="small" title={<div className="flex items-center gap-2"><span>✨ AI Hỗ trợ viết mô tả</span><TokenCostBadge cost={AI_TOKEN_COST} /></div>} style={{ marginBottom: 16, background: "color-mix(in oklch, var(--cosmate-info) 6%, var(--background))", borderColor: "color-mix(in oklch, var(--cosmate-info) 35%, var(--border))" }}>
          {isAiGenerating && <div style={{ marginBottom: 16 }}><AILoadingMascot type="content" variant="inline" /></div>}
          ...
          <Form.Item label="Chọn phong cách mô tả" style={{ marginBottom: 16 }}>
            <Radio.Group value={personaId} onChange={(event) => setPersonaId(event.target.value)} optionType="button" buttonStyle="solid">
          ...
          <Row justify="end"><Col><Button type="primary" icon={<RobotOutlined />} onClick={handleGenerateDescription} loading={isAiGenerating} disabled={disabled || loading || isAiGenerating || !canGenerateAI}>AI tự viết mô tả</Button></Col></Row>
        </Card>

        <Form.Item label="Kích cỡ" name="size"><Select placeholder="Chọn kích cỡ">{SIZE_OPTIONS.map((s) => <Select.Option key={s} value={s}>{s}</Select.Option>)}</Select></Form.Item>
        <Row gutter={12}><Col span={12}><Form.Item label="Chiều cao tối thiểu (cm)" name="heightMin"><InputNumber min={1} style={{ width: '100%' }} placeholder="Ví dụ: 145" /></Form.Item></Col><Col span={12}><Form.Item label="Chiều cao tối đa (cm)" name="heightMax"><InputNumber min={1} style={{ width: '100%' }} placeholder="Ví dụ: 155" /></Form.Item></Col></Row>
        <Row gutter={12}><Col span={12}><Form.Item label="Cân nặng tối thiểu (kg)" name="weightMin"><InputNumber min={1} style={{ width: '100%' }} placeholder="Ví dụ: 40" /></Form.Item></Col><Col span={12}><Form.Item label="Cân nặng tối đa (kg)" name="weightMax"><InputNumber min={1} style={{ width: '100%' }} placeholder="Ví dụ: 55" /></Form.Item></Col></Row>
        <Form.Item label="Số lượng vật phẩm (bao gồm tất cả vật phẩm và trang phục)" name="numberOfItems"><InputNumber min={1} style={{ width: '100%' }} placeholder="Số lượng vật phẩm" /></Form.Item>
        <Form.Item label="Giá thuê / ngày (VNĐ)" name="pricePerDay" rules={[{ required: true, message: 'Vui lòng nhập giá thuê' }]}><InputNumber min={1} style={{ width: '100%' }} placeholder="Giá thuê mỗi ngày" /></Form.Item>
        <Form.Item label="Tiền đặt cọc (VNĐ)" name="depositAmount"><InputNumber min={0} style={{ width: '100%' }} placeholder="Tiền đặt cọc" /></Form.Item>
        <Form.Item label="Hình ảnh" name="imageFiles" valuePropName="imageFiles"><Dragger multiple beforeUpload={() => false} accept="image/*" listType="picture"><p className="ant-upload-drag-icon"><InboxOutlined /></p><p className="ant-upload-text">Kéo thả hoặc nhấn để tải ảnh lên</p><p className="ant-upload-hint">Hỗ trợ tải nhiều ảnh cùng lúc</p></Dragger></Form.Item>
        <Form.Item><Button type="primary" htmlType="submit" loading={loading} block>Tiếp theo →</Button></Form.Item>
      </Form>
    </>
  )
}
