...
import { ProviderWalletTopUpContent } from '@/features/provider/components/ProviderWalletTopUpContent';
import { useWalletTopUp, type PaymentMethod } from '../hooks/useWalletTopUp';

export type WalletTopUpHubContentProps = {
  walletBase: string;
  redirectUrl?: string;
...
export function WalletTopUpHubContent({
  walletBase,
  redirectUrl,
  variant = 'cosplayer',
}: WalletTopUpHubContentProps) {
  const navigate = useNavigate();

  if (variant === 'provider') {
    return (
      <section className="mx-auto w-full max-w-4xl py-2">
        <ProviderWalletTopUpContent walletBase={walletBase} redirectUrl={redirectUrl} />
      </section>
    );
  }
